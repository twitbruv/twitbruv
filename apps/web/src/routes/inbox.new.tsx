...
function NewConversation() {
  ...
  function add(u: PublicUser) {
    setSelected((prev) => (prev.some((s) => s.id === u.id) ? prev : [...prev, u]))
    setQ("")
    setResults([])
  }
  ...
  return (
    <main>
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/80 px-4 py-3 backdrop-blur-sm">
        ...
        <Button size="sm" disabled={selected.length === 0 || busy} onClick={start}>
        ...
        {selected.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selected.map((u) => (
              <span
                key={u.id}
                className="flex items-center gap-1.5 rounded-full bg-muted py-1 pl-1 pr-2 text-xs"
              >
                <Avatar
