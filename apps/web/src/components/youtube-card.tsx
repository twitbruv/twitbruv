...
function VideoCardInner({
  ...
  return (
    <div
      className={cn(
        "relative",
        card.isShort && "mx-auto max-w-[280px]",
        className
      )}
    >
      <div
        className={cn(unfurlCardChromeClasses, "p-0", card.isShort && "mt-3")}
      >
        <div
          className={cn(
            "relative w-full bg-black",
            card.isShort ? "aspect-[9/16]" : "aspect-video"
          )}
        >
          {card.thumbnailUrl ? (
            <img
        ...
        >
          <div className="flex items-center gap-2 text-[11px] text-tertiary">
            ...
            {card.channelTitle && (
              <span className="truncate">{card.channelTitle}</span>
            )}
          </div>
      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <div className="flex flex-wrap gap-3 text-[11px] text-tertiary">
            ...
            {card.videoCount != null && (
              <span>{compactNumber(card.videoCount)} videos</span>
            )}
          </div>
        </div>
      </div>
