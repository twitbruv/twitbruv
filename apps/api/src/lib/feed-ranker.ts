...
export async function callForYouRanker({
  ...
  if (!response.ok) {
    log.warn(
      { status: response.status, userId: request.userId },
  ...
  try {
    ...
    log.warn({ err: errMsg(err) }, 'feed_ranker_invalid_json')
    return { kind: 'unavailable', reason: 'invalid_response' }
  }

  const data = parseRankerResponse(body)
  if (!data) {
    log.warn({ userId: request.userId }, 'feed_ranker_invalid_response_shape')
    return { kind: 'unavailable', reason: 'invalid_response' }
  }
...
function parseRankerResponse(value: unknown): ForYouRankResponse | null {
  ...
  if (!v.postIds.every((id) => typeof id === 'string' && id.length > 0)) return null
